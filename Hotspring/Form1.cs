    ...
    public partial class Form1 : Form
    {
        ...
        private void button_play_Click(object sender, EventArgs e)
        {
            ...
            if (start_button == true)
            {
                ...
                if (textBox_startValue.Text != "" || textBox_endValue.Text != "" || textBox_step.Text != "")
                {
                    ...
                    if (ini12.INIRead(Config_Path, "serialPort1", "Exist", "") == "1")          //送至Comport
                    {
                        ...
                        {
                            frontdata = "set RA ";
                            send_hotspring_command(startvalue, endvalue, step, frontdata, receive_command);
                        }
        ...
        private void serialPort1_recorder(byte ch, bool SaveToLog = false)
        {
            if ((ch == 0x0A) || (byteMessage_length_A >= byteMessage_max_Ascii))
            {
                ...
                if (receive != 0)
                    serialPort1_csv = string.Concat(serialPort1_csv, receive + ";");
        ...
        private void serialPort2_recorder(byte ch, bool SaveToLog = false)
        {
            if ((ch == 0x0A) || (byteMessage_length_B >= byteMessage_max_Ascii))
            {
                ...
                double receive = double.Parse(dataValue, CultureInfo.InvariantCulture);
                if (receive != 0)
                    serialPort2_csv = string.Concat(serialPort2_csv, receive + ";");
        ...
        private void send_hotspring_command(uint startvalue, uint endvalue, uint step, string frontdata, string recevice_command)
        {
            if (startvalue > endvalue)
            {
                for (uint i = startvalue; i <= endvalue; i += step)
                {
                    try
                    {
                        ...
                        string.Concat(serialPort1_csv, send_data + ";");
        ...
        private void Output_csv_log()
        {
            ...
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@".\log\" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".csv", true))
            {
                string[] serialPort1_split_data = serialPort1_csv.Split(';');
                string[] serialPort2_split_data = serialPort2_csv.Split(';');
                for (int i=0;i < serialPort1_split_data.Length; i++)
                {
